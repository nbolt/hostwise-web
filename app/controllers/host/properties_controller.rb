class Host::PropertiesController < Host::AuthController
  ...
  def show
    respond_to do |format|
      ...
      format.json { render json: property.to_json(include: [:property_photos, bookings: {include: [:services], methods: [:cost]}], methods: [:nickname, :short_address, :primary_photo, :full_address, :next_service_date]) }
    end
  end
  ...
  def booking_cost
    ...
    cost[:first_booking_discount] = if Booking.by_user(current_user)[0] then false else true end
    render json: cost
  end
