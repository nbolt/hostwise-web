class Contractor::TraineeController < Contractor::AuthController
  ...
  def available_jobs
    jobs = Job.standard.future.trainers.where(training: false,priority:1).order('bookings.date').to_a.uniq{|job| job.booking.date}
  ...
  def claim_jobs
    ...
    if jobs.find {|job| job.training}
      ...
      jobs.each do |job|
        ...
        distribution_job = current_user.jobs.build(distribution: true, status_cd: 1, priority: 0, booking_id: job.booking.id)
