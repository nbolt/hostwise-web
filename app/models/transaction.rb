class Transaction < ActiveRecord::Base
  ...
  def self.completed(user, start_date, end_date)
    if start_date.present? && end_date.present?
      Transaction.where(status_cd: 0).where('transactions.created_at >= ? and transactions.created_at <= ?', DateTime.strptime(start_date, '%m/%d/%Y'), DateTime.strptime(end_date, '%m/%d/%Y')).includes({bookings: [{property: :user}]}).references(:user, :bookings).where('users.id = ?', user.id)
    else
