class Job < ActiveRecord::Base
  ...
  def self.revenue_on_month date
    ...
    jobs.reduce(0) {|acc, job| acc + (job.chain(:booking, :prediscount_cost) || 0)} + Property.purchased.reduce(0) {|acc, property| acc + (property.linen_purchase_revenue || 0)}
  end
  ...
  def payout contractor=nil
    if booking
      if booking.cancelled? || booking.couldnt_access?
        ...
        payout += PRICING['laundry_payout'] * booking.linens_cost if booking.linen_handling == :in_unit
