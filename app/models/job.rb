class Job < ActiveRecord::Base
  ...
  scope :open, -> (contractor) {
    ...
    visible.standard.days(contractor).where(state_cd: states).where('(contractor_jobs.user_id is null or contractor_jobs.user_id != ?) and date >= ?', contractor.id, Date.today).order('date ASC').includes(:contractor_jobs).references(:contractor_jobs)
  }
