AdminJobsCtrl = ['$scope', '$http', '$timeout', 'spinner', ($scope, $http, $timeout, spinner) ->
  ...
  $scope.fetch_jobs = ->
    table = angular.element("#example-1").dataTable({
      ...
      aoColumns: [null,null,null,{bSortable:false},{bSortable:false},{bSortable:false},null,{bSortable:false},{bSortable:false},null,{bSortable:false},null,null,{bSortable:false},null,{bSortable:false},{bSortable:false},{bSortable:false},{bSortable:false},{bSortable:false},{bSortable:false},{bSortable:false}]
      serverSide: true
      fnInitComplete: ->
      ...
      ajax: (data, cb, settings) ->
        $http.get('/jobs.json',{params: {dash: 'jobs', search: $scope.search, filter: $scope.filter.id, data: data}}).success (rsp) ->
          $scope.jobs = rsp.jobs
          ...
          # show staging jobs
          data_jobs = _($scope.jobs).map (job) -> ["<a href='/jobs/#{job.id}' class='teal'>#{job.id}</a>", "<a href='/properties/#{job.booking.property.id}' class='teal'>#{job.booking.property.id}</a>", job.formatted_time, job.booking.property.zip_code.market.name, job.booking.property.property_size, (job.linen_handling || null), job.display_date, job.booking.property.nickname, job.booking.property.neighborhood_address, "<a href='/hosts/#{job.booking.user.id}/edit' class='teal'>#{job.booking.user.name}</a>", job.booking.property.display_phone_number, job.status, job.contractor_names, "$#{job.booking.cost}", job.booking.service_list, job.king_bed_count, job.twin_bed_count, job.toiletry_count, job.booking.extra_king_sets, job.booking.extra_twin_sets, job.booking.extra_toiletry_sets, job.state]
