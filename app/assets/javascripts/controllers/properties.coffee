  ...
  $scope.quick_add = (property) ->
    ...
    $http.get("/properties/#{property.slug}.json").success (rsp) ->
      ...
      _($scope.property.active_bookings).each (booking) ->
        ...
        angular.element(".booking.modal .calendar td.day[month=#{date.month()+1}][year=#{date.year()}][day=#{date.date()}]").removeClass('active').addClass('inactive').addClass('booked').attr('booking', booking.id)
